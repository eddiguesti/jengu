// backend/src/scrapers/SanaryCampingScraper.ts
...
export class SanaryCampingScraper {
  ...
  private async scrapeVacancesCampings(): Promise<CampsiteData[]> {
    ...
    try {
      const searchUrl = 'https://www.vacances-campings.fr/camping-provence-alpes-cote-d-azur/camping-var/camping-bord-de-mer';
  ...
  private async scrapeCampingFr(): Promise<CampsiteData[]> {
    ...
    try {
      ...
      // Handle cookies
      try {
        const cookieButton = await page.$('#didomi-notice-agree-button, .cookie-accept, [class*="accept"]');
        if (cookieButton) await cookieButton.click();
      } catch {}

      // Wait for results
      try {
        await page.waitForSelector('.search-result-item, .campsite-card, .result-item', { timeout: 10000 });
      } catch {
        ...
        const priceInfo = await page.evaluate(() => {
          ...
          for (const pattern of pricePatterns) {
            ...
            if (match && match[1]) {
              ...
              if (parsedPrice > 10 && parsedPrice < 200) { // Reasonable price range
                price = parsedPrice;
                break;
              }
            }
