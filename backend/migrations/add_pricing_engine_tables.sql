...
CREATE POLICY "select_own_quotes" ON pricing_quotes FOR SELECT USING (auth.uid() = "userId"::uuid);
CREATE POLICY "insert_own_quotes" ON pricing_quotes FOR INSERT WITH CHECK (auth.uid() = "userId"::uuid);

-- Pricing Outcomes Policies
CREATE POLICY "select_own_outcomes" ON pricing_outcomes FOR SELECT USING (auth.uid() = "userId"::uuid);
CREATE POLICY "insert_own_outcomes" ON pricing_outcomes FOR INSERT WITH CHECK (auth.uid() = "userId"::uuid);

-- Inventory Snapshots Policies
CREATE POLICY "select_own_inventory" ON inventory_snapshots FOR SELECT USING (auth.uid() = "userId"::uuid);
CREATE POLICY "insert_own_inventory" ON inventory_snapshots FOR INSERT WITH CHECK (auth.uid() = "userId"::uuid);
